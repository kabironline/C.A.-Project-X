...
class Cart {
	...
	public static void cart(){
		...
		if (Mycart.size() == 0) {
			...
			for (int i = 0;i <= Mycart.size()-1 ;i++ ) {
				...
				if (Mycart.get(i).powerStorage  == 0) {
					PowerStorage = "";
				}
				if (Mycart.get(i).storage  == 0) {
					Storage = "";
				}
				if (Mycart.get(i).RAM  == 0) {
					Ram = "";
				}
	...
	public static void addItemsToCartAcc (){
		...
		while(!exit){
			...
			if (ProjectK.intYouNeed == 999) {
				ProjectK.home();
			}else if ((ProjectK.intYouNeed >= 0) && (ProjectK.intYouNeed < accessoriesList.list.size()) ){
				String Name = list.get(ProjectK.intYouNeed).name;
				int Price = list.get(ProjectK.intYouNeed).price;
				double RAM = list.get(ProjectK.intYouNeed).RAM;
				int Storage = list.get(ProjectK.intYouNeed).storage;
	...
	public static void itemQuantityAdderAcc (Cart item) {
		boolean exit = false;
		boolean exit2 = false;
		accessoriesList list = new accessoriesList();
		String Name = item.name;
